# -*- coding: utf-8 -*-
"""
Created on Fri Apr 28 18:18:13 2023

@title: Lab Session 6 Task 4  Part 2: Integrating Control of the Robot Arm
...
def define_initial_elements(arm_dimensions):
    ...
    T1 = Ry(-pi/2) * T(arm_dimensions[0], 0, 0) * Rx(theta1)
    T2 = T1 * T(arm_dimensions[1], 0, 0) * Rz(theta2)
    T3 = T2 * T(arm_dimensions[2], 0, 0) * Rz(theta3)
    T4 = T3 * T(arm_dimensions[3], 0, 0) * Rx(theta4)
    T5 = T4 * T(arm_dimensions[4], 0, 0) * Rz(theta5)
    T6 = T5 * T(arm_dimensions[5], 0, 0) * Rx(theta6)
...
def calculate_joint_angles(elements, initial_pose, final_position, position_type, simulate):
    print("Calculating final robot pose for " + position_type + " position: ",
          final_position, " from initial pose: ", initial_pose, end="")
    
    # Initial position θi to start movement from 
    theta_i = Matrix(initial_pose);
    
    theta1, theta2, theta3, theta4, theta5, theta6 = elements[0:6]
    ...
    while dp.norm() > dp_threshold:
        ...
        elements, X, Y, Z, W = calculate_next_position(elements, step_size, theta_max_step)
        dp = elements[-1]
        theta_i = elements[7]
        p_i = elements[-3]
...
def calculate_next_position(elements, step_size, theta_max_step):
    ...
    [theta1, theta2, theta3, theta4, theta5, theta6, theta, theta_i,
     p0, p1, p2, p3, p4, p5, p6, p, p_i, p_f, dp] = elements;
...
def main():
    ...
    elements = define_initial_elements(arm_dimensions);
    
    initial_pose = [0, 1, -2.5, 0, -1.6, 0] # [0,0,0,0,0,0]; # or [-pi/2,-pi/4,3*pi/4,0,pi/2,0];
    final_position = [-0.05, -0.01, 0.09];
    
    theta_i = calculate_joint_angles(elements, initial_pose, final_position, position_type = "relative", simulate = False)
    
    initial_pose = theta_i
